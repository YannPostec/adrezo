...
<head>
<title><fmt:message key="photo.room.title" /></title>
...
<script type="text/javascript" charset="utf-8" src="../js/photos_salles.js"></script>
...
<sql:query var="salles">select * from salles_display where ctx=${validUser.ctx} order by site_name,name</sql:query>
...
<table><thead><tr><th /><th><fmt:message key="admin.site" /></th><th><fmt:message key="common.table.name" /></th></tr>
<tobdy><tr>
	...
	<td><select id="add_site"><option value="0"><fmt:message key="common.select.site" /></option><c:forEach items="${sites.rows}" var="site"><option value="${site.id}">${site.name}</option></c:forEach></select></td>
	<td><input type="hidden" id="add_id" value="" /><input type="text" size="6" id="add_name" value="" /></td>
</tr></tbody></table>
<h3><fmt:message key="photo.room.list" /> :</h3>
<div id="tablewrapper">
	<div id="tableheader">
  	<div class="search">
	...
	<table id="table" class="tinytable">
		<thead><tr><th class="nosort"><h3 class="nosearch" /></th><th><h3><fmt:message key="admin.site" /></h3></th><th><h3><fmt:message key="common.table.name" /></h3></th></tr></thead>
		<tbody><c:forEach items="${salles.rows}" var="salle"><tr>
			<td style="text-align:center"><span onmouseover="javascript:tooltip.show('${lang_commonclickdel}')" onmouseout="javascript:tooltip.hide()"><img src="../img/icon_delete.jpg" alt="${lang_commonclickdel}" onclick="javascript:ConfirmDlg(${salle.id})" /></span> <span onmouseover="javascript:tooltip.show('${lang_commonclickmod}')" onmouseout="javascript:tooltip.hide()"><img src="../img/icon_modify.jpg" alt="${lang_commonclickmod}" onclick="javascript:sendModif(event)" /></span></td>
			<td><input type="hidden" value="${salle.idsite}" />${salle.site_name}</td>
			<td><input type="hidden" value="${salle.id}" />${salle.name}</td>
		</tr></c:forEach></tbody>
	</table>
	<div id="tablefooter">
		<div id="tablenav">
