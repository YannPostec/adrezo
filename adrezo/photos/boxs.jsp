...
<head>
<title><fmt:message key="photo.box.title" /></title>
...
<script type="text/javascript" charset="utf-8" src="../js/photos_boxs.js"></script>
...
<sql:query var="boxs">select * from photo_box_display where ctx=${validUser.ctx} order by site_name,salle_name,name</sql:query>
<h3><fmt:message key="admin.mgt" /> :</h3>
<table><thead><tr><th /><th><fmt:message key="admin.site" /></th><th><fmt:message key="common.table.room" /></th><th><fmt:message key="common.table.name" /></th></tr></thead>
<tbody><tr>
	...
	<td><select id="add_salle"><option value="0"><fmt:message key="common.select.room" /></option></select></td>
	<td><input type="hidden" id="add_id" value="" /><input type="text" size="50" id="add_name" value="" /></td>
</tr></tbody></table>
<h3><fmt:message key="photo.box.list" /> :</h3>
<div id="tablewrapper">
	<div id="tableheader">
  	<div class="search">
	...
	<table id="table" class="tinytable">
		<thead><tr><th class="nosort"><h3 class="nosearch" /></th><th><h3><fmt:message key="admin.site" /></h3></th><th><h3><fmt:message key="common.table.room" /></h3></th><th><h3><fmt:message key="common.table.name" /></h3></th></tr></thead>
		<tbody><c:forEach items="${boxs.rows}" var="box"><tr>
			<td style="text-align:center"><span onmouseover="javascript:tooltip.show('${lang_commonclickdel}')" onmouseout="javascript:tooltip.hide()"><img src="../img/icon_delete.jpg" alt="${lang_commonclickdel}" onclick="javascript:ConfirmDlg(${box.id})" /></span> <span onmouseover="javascript:tooltip.show('${lang_commonclickmod}')" onmouseout="javascript:tooltip.hide()"><img src="../img/icon_modify.jpg" alt="${lang_commonclickmod}" onclick="javascript:sendModif(event)" /></span></td>
			<td><input type="hidden" value="${box.idsite}" />${box.site_name}</td>
			<td><input type="hidden" value="${box.idsalle}" />${box.salle_name}</td>
			<td><input type="hidden" value="${box.id}" />${box.name}</td>
		</tr></c:forEach></tbody>
	</table>
	<div id="tablefooter">
		<div id="tablenav">
