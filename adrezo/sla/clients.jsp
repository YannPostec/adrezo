...
<head>
<title><fmt:message key="sla.client.title" /></title>
...
<tr>
<td><span onmouseover="javascript:tooltip.show('${lang_commonclickdel}')" onmouseout="javascript:tooltip.hide()"><img src="../img/icon_delete.jpg" alt="${lang_commonclickdel}" onclick="javascript:ConfirmDlg(${client.ID})" /></span></td>
...
<td><input type="hidden" value="${client.ID}" />${client.NAME}</td>
<td><c:choose><c:when test="${client.DISP == 1}"><fmt:message key="common.yes" /></c:when><c:otherwise><fmt:message key="common.no" /></c:otherwise></c:choose></td>
<td><select style="display:none"><option><fmt:message key="common.select.plan" /></option><c:forEach items="${plans.rows}" var="plan"><option value="${plan.id}"<c:if test="${plan.id == client.plan}"> selected="selected"</c:if>>${plan.id}-<c:choose><c:when test="${plan.id == 0}"><fmt:message key="sla.planning.anytime" /></c:when><c:otherwise>${plan.name}</c:otherwise></c:choose></option></c:forEach></select><input type="hidden" value="${client.plan}" /><c:choose><c:when test="${client.plan == 0}"><fmt:message key="sla.planning.anytime" /></c:when><c:otherwise>${client.plan_name}</c:otherwise></c:choose></td></tr>
</c:forEach>
</tbody>
</table>
<div id="dlgcontent"/>
