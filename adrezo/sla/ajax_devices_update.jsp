...
<c:when test="${validUser != null && pageContext.request.method == 'POST' && !empty param.id && validUser.rezo}">
	...
	<c:choose>
		<c:when test="${errUpdate != null}">
			...
			<c:set var="message" scope="page">${message}<erreur>true</erreur><msg><fmt:message key="sla.movedevice" /> : <fmt:message key="common.error" />, <adrezo:trim value="${errUpdate}"/></msg></c:set>
		</c:when>
		<c:otherwise>
			<c:set var="message" scope="page">${message}<erreur>false</erreur><msg><fmt:message key="sla.movedevice" /> : <fmt:message key="common.ok" /></msg></c:set>
		</c:otherwise>
	</c:choose>	
</c:when>
<c:otherwise>
