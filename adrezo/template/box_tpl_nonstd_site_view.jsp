...
<p><fmt:message key="common.table.name" /> : ${site.name}<br />
<fmt:message key="template.site.nonstd" />
</p>
</c:forEach>
<hr />
...
<table><thead><tr><th><fmt:message key="common.table.name" /></th><th><fmt:message key="common.table.ip" /></th><th><fmt:message key="common.table.mask" /></th><th><fmt:message key="common.table.ipgw" /></th><th><fmt:message key="admin.vlan" /></th><th><fmt:message key="common.table.ipbc" /></th><th><fmt:message key="norm.surnet" /></th></tr></thead><tbody>
<c:forEach items="${subnets.rows}" var="subnet">
<sql:query var="surnets">select ip,mask,def from surnets where id in (${subnet.surnet})</sql:query>
<tr>
	<td>${subnet.def}</td>
	...
	<td><c:forEach items="${surnets.rows}" var="surnet"><div><adrezo:displayIP value="${surnet.ip}" />/${surnet.mask} (${surnet.def})</div></c:forEach></td>
</tr>
</c:forEach>
</tbody></table>
<p>&nbsp;</p>
