...
<c:choose>
<c:when test="${validUser != null && pageContext.request.method == 'POST' && validUser.admin && !empty param.id && !empty param.login && !empty param.subnets}">
<c:set var="message"><valid>true</valid></c:set>
<c:choose>
	<c:when test="${empty param.update}">
		<c:catch var="errInsert">
			<sql:update>
				insert into api (id,login,subnets) values (?${adrezo:dbCast('INTEGER')},?,?)
				<sql:param value="${param.id}"/>
				<sql:param value="${param.login}"/>
				<sql:param value="${param.subnets}"/>
			</sql:update>
		</c:catch>
		<c:choose>
