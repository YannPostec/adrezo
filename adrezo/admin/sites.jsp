...
<sql:query var="sites">select * from sites_display where ctx=${validUser.ctx} order by name</sql:query>
<h3><fmt:message key="admin.mgt" /> :</h3>
<table><thead><tr><th /><th><fmt:message key="common.table.codesite" /></th><th><fmt:message key="common.table.name" /></th></tr></thead>
<tbody><tr>
	...
	<td><input type="hidden" id="add_id" value="" /><input type="text" size="8" id="add_cod" value="" /></td>
...
<div id="tablewrapper">
	...
	<table id="table" class="tinytable">
		<thead><tr><th class="nosort"><h3 class="nosearch" /></th><th><h3><fmt:message key="common.table.codesite" /></h3></th><th><h3><fmt:message key="common.table.name" /></h3></th></tr></thead>
		<tbody>
		<c:forEach items="${sites.rows}" var="site">
			<tr>
				<td style="text-align:center"><span onmouseover="javascript:tooltip.show('${lang_commonclickdel}')" onmouseout="javascript:tooltip.hide()"><img src="../img/icon_delete.jpg" alt="${lang_commonclickdel}" onclick="javascript:ConfirmDlg(${site.id})" /></span> <span onmouseover="javascript:tooltip.show('${lang_commonclickmod}')" onmouseout="javascript:tooltip.hide()"><img src="../img/icon_modify.jpg" alt="${lang_commonclickmod}" onclick="javascript:sendModif(event)" /></span></td>
				<td><input type="hidden" value="${site.id}" />${site.cod_site}</td>
				<td>${site.name}</td>
			</tr>
		</c:forEach>
	</tbody></table>
