...
<c:choose><c:when test="${param.type == 'site'}"><sql:query var="subnets">select id,ip,mask,def from subnets where site=${param.id} and surnet=0 order by def</sql:query></c:when>
<c:otherwise><sql:query var="subnets">select id,ip,mask,def from subnets where ctx=${param.id} and surnet=0 order by def</sql:query></c:otherwise></c:choose>
