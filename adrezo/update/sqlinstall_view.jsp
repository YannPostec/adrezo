	...
	<c:when test="${pageContext.request.method == 'POST' && fn:contains(header.referer,'install.jsp') }">
		...
		<c:catch var="err">
			<sql:transaction>
				...
				<sql:update>create view stock_etat_display (id,def,stock,seuil,idx,cat,idcat,encours,ctx,ctx_name,site,site_name) as select e.id,e.def,e.stock,e.seuil,e.idx,c.name,c.id,e.encours,e.ctx,x.name,e.site,s.name from stock_etat e,stock_cat c,contextes x,sites s where e.cat=c.id and e.ctx=x.id and e.site=s.id<c:if test="${adrezo:envEntry('db_type') == 'oracle'}"> with read only</c:if></sql:update>
				...
				<sql:update>create view vlan_display (id,vid,def,site,ctx,cod_site,site_name,ctx_name) as select v.id,v.vid,v.def,v.site,v.ctx,s.cod_site,s.name as site_name,c.name as ctx_name from vlan v,sites s,contextes c where v.site=s.id and v.ctx=c.id<c:if test="${adrezo:envEntry('db_type') == 'oracle'}"> with read only</c:if></sql:update>
			</sql:transaction>
		</c:catch>
		<c:choose>
