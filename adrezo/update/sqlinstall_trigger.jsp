...
<c:choose>
	<c:when test="${pageContext.request.method == 'POST' && fn:contains(header.referer,'install.jsp') }">
		...
		<c:catch var="err">
			...
			<adrezo:sqlTrigger>create trigger dhcp_server_del after delete on dhcp_server referencing new as new old as old for each row begin delete from dhcp_exclu where hostname = :old.hostname; end dhcp_server_del;</adrezo:sqlTrigger>
		...
		<c:catch var="err">
			<adrezo:sqlTrigger>create or replace function create_language_plpgsql() returns boolean as $BODY$ create language plpgsql; select true; $BODY$ language SQL;</adrezo:sqlTrigger>
			<adrezo:sqlTrigger>select case when not ( select true as exists from pg_language where lanname = 'plpgsql' union select false as exists order by exists desc limit 1 ) then create_language_plpgsql() else false end as plpgsql_created;</adrezo:sqlTrigger>
			<adrezo:sqlTrigger>drop function create_language_plpgsql();</adrezo:sqlTrigger>
			...
			<adrezo:sqlTrigger>create or replace function dhcp_server_del() returns trigger as $BODY$ begin delete from dhcp_exclu where hostname = OLD.hostname; return OLD; end; $BODY$ LANGUAGE plpgsql;</adrezo:sqlTrigger>
