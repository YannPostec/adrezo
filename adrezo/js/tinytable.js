...
TINY.table=function(){
	...
	sorter.prototype.init=function(){
		...
		if(this.p.colddid){
			d=T$(this.p.colddid);
			var o=document.createElement('option'); o.value=-1; o.innerHTML=langdata.all; d.appendChild(o)
		}
		for(i;i<t.w;i++){
			...
			if(d && T$$('h3',c)[0].className!='nosearch'){
	...
	sorter.prototype.search=function(f){
		...
		for(i;i<this.t.l;i++){
			...
			if(q==''){
				...
				for(x=s;x<e;x++){
					var b=r.cells[x].textContent.toLowerCase();
	...
	function FrDate(dd) {var aa = dd.split(/[\\\/]/);return aa[2]+aa[1]+aa[0]}
	function cp(f,c){
		var g,h; f=g=f.v.toLowerCase(); c=h=c.v.toLowerCase();
		var i=parseFloat(f.replace(/(\$|\,)/g,'')), n=parseFloat(c.replace(/(\$|\,)/g,''));
		if(!isNaN(i)&&!isNaN(n)){g=i;h=n}
		if(isNaN(f)&&isNaN(c)&&f.length==10&&f.lastIndexOf("/")>0){g=FrDate(f);h=FrDate(c)}
		if(isNaN(f)&&isNaN(c)&&f.length>9&&f.length<19&&c.length>9&&c.length<19&&f.lastIndexOf(".")>0&&f.lastIndexOf("/")>0&&c.lastIndexOf(".")>0&&c.lastIndexOf("/")>0){g=renderip(f.substring(0,f.lastIndexOf("/")));h=renderip(c.substring(0,c.lastIndexOf("/")))}
		else if(isNaN(f)&&isNaN(c)&&f.length>6&&f.length<16&&c.length>6&&c.length<16&&f.lastIndexOf(".")>0&&c.lastIndexOf(".")>0){g=renderip(f);h=renderip(c)}
