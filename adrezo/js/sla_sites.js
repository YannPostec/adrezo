...
function verifyInput(name,selplan,selclient) {
	var strAlert="";
...
function addSubmit() {
	...
	var strAlert = verifyInput(name,selplan,selclient);
	if (strAlert != "") {
		showDialog(langdata.invalidfield+" :",strAlert,"warning",0,1);
	} else {
		DBAjax("ajax_sites_store.jsp","id=&client="+client+"&name="+name+"&disp="+disp+"&plan="+plan);
	}
}
function modSubmit(e) {
	...
	if (strAlert != "") {
		...
		DBAjax("ajax_sites_store.jsp","id="+id+"&client="+client+"&name="+name+"&disp="+disp+"&plan="+plan,true,function callback(result) {
...
function ApplyModif(id) {
	...
	var hid1 = tds[5].firstChild.nextSibling
	var hid2 = tds[5].firstChild.nextSibling.nextSibling;
	var texte = ipt.options[ipt.selectedIndex].text;
	var txt = document.createTextNode(texte.substring(texte.indexOf("-")+1,texte.length));
	tds[5].replaceChild(txt,hid2);
	hid1.value=ipt.value;
	ipt.style.display = "none";
	refreshTable(false);
}
			...
			if (erreur == "true") { createP(msg); }
				...
				for (var i=0;i<lines.length;i++) {
					...
					if (T$$("client",lines[i])[0].hasChildNodes() && T$$("client_name",lines[i])[0].hasChildNodes()) {
						...
						var mysel = T$("add_client").cloneNode(true);
...
function SwitchSearch(i) {
		switch(i) {
			case 2: T$("sqs_order").value="client";break;
			case 3: T$("sqs_order").value="name";break;
			case 4: T$("sqs_order").value="disp";break;
			case 5: T$("sqs_order").value="plan";break;
		}
}
