...
public class MailMigIPJob implements Job {
	private String lang = "";
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		...
		try {
			...
			rsg = stmt.executeQuery("select * from schedulers where id=5");
			if (rsg.next()) {
				if (rsg.getInt("enabled")>0) {
					...
					while (rsmail.next()) {
						...
						rsuser = stmtUser.executeQuery("select ctx_name,subnet_name,site_name,name,ip,mask,date_mig from adresses_display where usr_mig='"+myuser+"' and mig = 1 and type = 'static' and date_mig<"+DbFunc.ToDateStr()+"('"+new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new java.util.Date())+"','YYYY-MM-DD HH24:MI:SS') order by date_mig");
						List<String> messages = new ArrayList<String>();
						while (rsuser.next()) {
							String myfdate = "";
							if (db_type.equals("oracle")) { myfdate=new java.text.SimpleDateFormat("yyyy-MM-dd").format(rsuser.getDate("date_mig")); }
    					if (db_type.equals("postgresql")) { myfdate=new java.text.SimpleDateFormat("yyyy-MM-dd").format(rsuser.getTimestamp("date_mig")); }
						...
						if (!messages.isEmpty()) {
							rsuser = stmtUser.executeQuery("select * from mail where id=3 and lang='"+userlang+"'");
							if (rsuser.next()) {
					...
					String listusers="";
					rsadmin = stmt.executeQuery("select distinct(usr_mig) as UserList from adresses where mig = 1 and type = 'static' and date_mig<"+DbFunc.ToDateStr()+"('"+new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new java.util.Date())+"','YYYY-MM-DD HH24:MI:SS') and usr_mig not in (select login from usercookie)");
					while (rsadmin.next()) {
					...
					if (!listusers.equals("")) {
						...
						List<String> messages = new ArrayList<String>();
						rsadmin = stmt.executeQuery("select ctx_name,subnet_name,site_name,name,ip,mask,usr_mig,date_mig from adresses_display where usr_mig in ("+listusers+") and mig = 1 and type = 'static' and date_mig<"+DbFunc.ToDateStr()+"('"+new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new java.util.Date())+"','YYYY-MM-DD HH24:MI:SS') order by date_mig");
						while (rsadmin.next()) {
							String myfdate = "";
							if (db_type.equals("oracle")) { myfdate=new java.text.SimpleDateFormat("yyyy-MM-dd").format(rsadmin.getDate("date_mig")); }
    					if (db_type.equals("postgresql")) { myfdate=new java.text.SimpleDateFormat("yyyy-MM-dd").format(rsadmin.getTimestamp("date_mig")); }
							messages.add(rsadmin.getString("usr_mig")+" : IP "+IPFmt.displayIP(rsadmin.getString("ip"))+"/"+rsadmin.getString("mask")+" , "+rsadmin.getString("name")+" , [ "+rsadmin.getString("ctx_name")+" | "+rsadmin.getString("site_name")+" | "+rsadmin.getString("subnet_name")+" ] , "+myfdate);
						}
						if (!messages.isEmpty()) {
							rsadmin = stmt.executeQuery("select * from mail where id=3 and lang='"+this.lang+"'");
							if (rsadmin.next()) {
				...
				} else mylog.debug("Job 5 disabled");
			}
			rsg.close();rsg=null;
			stmt.close();stmt=null;
			conn.close();conn=null;
		} catch (Exception e) { mylog.error("MailMigIP/DB: "+e.getMessage(),e); }
		finally {    
